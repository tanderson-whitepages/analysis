...
i = input('choose 0-'+str(len(headers))+'>')
...
for x in resolutionSet:
  ...
  while foo != '0' and foo != '1':
    foo = input('enter 0 if this is non-fraud, 1 if it is fraud>')
    if foo == '1':
...
if anynonnull:
  ...
  for x in cbSet:
    ...
    foo = None
    while foo != '0' and foo != '1':
      foo = input('enter 0 if this indicates not a chargeback, 1 if it is a chargeback>')
      if foo == '1':
